...
class Minifier {
    ...
    private $config_keys = array(
        ...
        'remove_comments' => true,      //Remove comments, 
 		'create_new' => false,          //GEOmin Ecrase les fichier existants
...
//<<GEOmin
			$fcredit = '../'.$sfs[0].(isset($sfs[1]) ? '/'.$sfs[1] : '').'/CREDITS.txt';
			if ( $sfs[0] != 'src' ) {
				if ( is_file ( $fcredit ) ) {
					$credits = explode ("\n", file_get_contents ( $fcredit ));
					$this->credits[isset($sfs[1]) ? $sfs[1] : $sfs[0]] = ' * '.$credits[0]."\n";
				} else
					echo "<p style='color:red'>Fichier non trouv&eacute;: $fcredit</p>";
			}
			echo "$src_file<br/>";
			$this->output .= "\n/**\n * $src_file (".round (strlen ($this->content) / 1024, 1)." kb)\n */\n";
...
//>>GEOmin
    ...
    {        
        ...
        if( !is_dir( dirname( $new_file ) ) || !is_writeable( dirname( $new_file ) ) )
        {
            ...
            echo ( dirname( $new_file ) . ' is not writable.  Cannot create minified file.' ); //GEOmin
            return false;
        }
...
if(0){//GEOmin
            ...
            $this->compiled = '';
            $this->credits = array ();
            $this->src = ''; // Fichier pointant sur les fichiers sources originaux
            $this->debug = ''; // Fichier pointant sur les fichiers sources copiés dans la distrib
            $this->prequel = '/**' . PHP_EOL.
				' * Integrated by Dominique Cavailhez (c) 2014' . PHP_EOL.
				' * https://github.com/Dominique92/MyLeaflet' . PHP_EOL. ' *' . PHP_EOL.
				' * Credits:' . PHP_EOL;
//>>GEOmin
            ...
            $this->prequel .= implode ('', $this->credits) . ' * ' . PHP_EOL.' * Generated by MagicMin: ' . date( 'Y-m-d h:i:s' ). PHP_EOL.' */' . PHP_EOL; //GEOmin
...
//<<GEOmin Création des fichiers includes de debug
		if ($this->extension == 'js')
			$this->prequel.= PHP_EOL.
				'var scripts = document.getElementsByTagName("script"),' . PHP_EOL.
				'	script = scripts[scripts.length - 1].src,' . PHP_EOL.
				'	racineSources = script.substring(0, script.lastIndexOf("/")) + "/";' . PHP_EOL;
		if ($this->extension == 'js')
			file_put_contents ('../dist/src/'.$this->filename.'.'.$this->extension, $this->prequel.$this->debug);
//>>GEOmin
        ...
        if( !$this->handle )
        {
            ...
            echo( 'Unable to open file:  '.$new_file ); //GEOmin
            return false;
        }
        else
        {
    ...
    {
//<<GEOmin
		echo '<br/><u>'.$output_filename.'</u>:<br/>';
		$this->output_directory = pathinfo( $output_filename, PATHINFO_DIRNAME );
		$this->credit = '';
        $this->filename  = strtolower( pathinfo( $output_filename, PATHINFO_FILENAME  ) );
        $this->extension = strtolower( pathinfo( $output_filename, PATHINFO_EXTENSION ) );
//>>GEOmin
        ...
        $this->create_new = $this->settings['create_new']; //GEOmin force l'ecrasement du fichier
//GEOmin        $this->create_new = false;
        ...
        if( !empty( $order ) )
        {
            ...
            foreach( $order as $this->file )
            {
                ...
                if( file_exists( '../'.$minified_name ) && 
					( $this->file != $output_filename ) && 
					( !$this->remote_file( $this->file ) ) && 
					( $this->gmstamp( filemtime( '../'.$this->file ) ) > $minfile->filemtime ) ||
					!in_array( $this->file, $minified_filelist ) )
                {
